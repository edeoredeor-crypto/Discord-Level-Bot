...
const SAFE_CHANNEL_ID = process.env.SAFE_CHANNEL_ID || "1454085014370390058";
...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    background TEXT DEFAULT '${DEFAULT_BACKGROUND}',
    remind_level INTEGER DEFAULT 0,
...
const utils = {
   ...
   */
  getRank(level) { 
    return RANKS.slice().reverse().find(r => level >= r.level) || RANKS[0]; 
  },

  /**
   * Create XP progress bar
   * @param {number} current - Current XP
   * @param {number} max - Max XP for level
   * @param {number} size - Size of bar
  ...
  /**
   * Send temporary message that deletes after 10 seconds (except in safe channel)
   ...
   * @returns {Object|null} Achievement object or null if error
   */
  ...
  async updateRankRole(member, newRank) {
    ...
    try {
      ...
      console.error("Error updating roles:", e.message);
    }
  },
   ...
   * @param {number} timestamp - Timestamp for cooldown
   */
  async giveXP(member, amount, timestamp = Date.now()) {
    ...
    db.prepare("INSERT OR IGNORE INTO users (user_id, guild_id, background) VALUES (?, ?, ?)")
      .run(userId, guildId, DEFAULT_BACKGROUND);
    ...
    if (leveledUp && newLevel === remindLevel) {
      try {
        await member.send(`🔔 **Level ${newLevel} achieved!** Your reminder has been triggered.`);
      } catch {}
    }
    ...
    if (leveledUp) {
      ...
      // Announce level up in system channel
      const channel = guild.systemChannel || guild.channels.cache.find(c => c.type === 0); // Find first text channel
      if (channel) {
        try {
  ...
  /**
   * Check if user has 7-day daily streak
   * @param {string} userId - User ID
   * @param {string} guildId - Guild ID
   * @returns {boolean} True if user has 7-day streak
   */
   ...
   * @param {string} giverId - ID of the user giving rep
   * @param {string} guildId - Guild ID
  ...
  /**
   * Generate profile image for a user
   * @param {GuildMember} member - Discord member
   * @param {Object} user - User data from database
   * @returns {Buffer|null} Image buffer or null if error
   */
...
const commands = {
  ...
  async info(message) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x55acee)
      .addFields(
        { name: "Bot Version", value: "3.1.0", inline: true },
...
// Show full profile with image or embed
